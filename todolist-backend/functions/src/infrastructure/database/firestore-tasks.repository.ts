...
export class FirestoreTasksRepository implements TasksRepository {
  private readonly tasksCollection = FirebaseConfig.getFirestore().collection(
    CommonConstants.TASKS_COLLECTION
  );

  async getTasksByUser(userId:string): Promise<Task[]> {
    logger.info("Entro a getTasksByUser: ", userId);

    const tasksSnapshot = await this.tasksCollection.where(
      CommonConstants.TASKS_USER_ID_FIELD, "==", userId
    ).get();
    ...
    if (tasksSnapshot.empty) {
      ErrorService.throwNotFound(
        UserErrorsConstants.TASKS_NOT_FOUND.internalCode,
        UserErrorsConstants.TASKS_NOT_FOUND.msg
      );
    }
  ...
  async updateTask(task: Task): Promise<Task> {
    ...
    if (!task.id) {
      ErrorService.throwValidationError(
        UserErrorsConstants.TASK_ID_FIELD_REQUIRED.internalCode,
        UserErrorsConstants.TASK_ID_FIELD_REQUIRED.msg
      );
    }
    ...
    if (!taskDoc.exists) {
      ErrorService.throwNotFound(
        UserErrorsConstants.TASK_NOT_FOUND.internalCode,
        UserErrorsConstants.TASK_NOT_FOUND.msg
      );
    }
  ...
  async deleteTask(id:string): Promise<void> {
    ...
    if (!doc.exists) {
      ErrorService.throwGeneralError(
        UserErrorsConstants.TASKS_DELETED_NOT_EXISTS.internalCode,
        UserErrorsConstants.TASKS_DELETED_NOT_EXISTS.msg
      );
    }
  ...
  private toDomain(doc: FirebaseFirestore.DocumentSnapshot): Task {
    ...
    return new Task(
      ...
      data.state,
      data.createdDate,
      CommonConstants.FIELD_NOT_DEFINED, doc.id
    );
  }
}
